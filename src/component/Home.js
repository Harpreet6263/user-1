...
const fetchMessageData = async () => {
  try {
      const response = await Axios.get('http://localhost:5000/messagedata');
...
useEffect(() => {
  if (messageData.length > 0) {
    ...
    setSender(messageData[messageData.length - 1].sender);
  }
}, [messageData]);
          ...
          <div className="chatParent">
            <div className="rightNavbar">
              <div className="rightNavbarleft">
                ...
                <p className="username">Priyansh</p>
              </div>
  ...
  return (
    ...
    <div className='container'>
      <div className="leftContainer">
        ...
        <div className="user" onClick={() => setChat('Priyansh')}>
          <div className="userPic">
            <img id='dp' src={profilePic} alt="" />
          </div>
          <div className="nameChat">
            <p id='senderName'>Priyansh</p>
            <p id='sendMsg'><img id='tick' src={sender === 'Harpreet' ? dot :tick} alt="" />{sender}: {lastMessage}</p>
          </div>
        </div>
      </div>
